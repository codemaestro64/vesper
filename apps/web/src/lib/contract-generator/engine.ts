...
export const registerGenerator = (gen: ContractGenerator) => {
...
export function generateContractCode(config: ContractConfig): string {
  ...
  const ctx: RenderContext = {
    ...
    access: (config.access ?? 'ownable') as AccessControl,
  }
...
function applyAccessControl(parts: ContractParts, ctx: RenderContext) {
  ...
  if (ctx.access === 'roles') {
    ...
    parts.constructorBody.unshift('_grantRole(DEFAULT_ADMIN_ROLE, msg.sender);')
