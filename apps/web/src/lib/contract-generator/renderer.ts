...
export const renderContract = (parts: ContractParts, baseContract: string): string => {
  ...
  const inheritance =
    parts.inheritances.length > 0
      ? ` is ${parts.inheritances.join(', ')}`
      : ''
